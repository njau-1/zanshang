...
$(function(){
    /*意见反馈表单提交*/
    $(".suggestion-textarea textarea").keydown(function(){
        if(this.value.length >= 200) {
            alert("反馈请不要超过100字");
        }
    });
    $(".suggestion-input input").keydown(function(){
        if(this.value.length >= 100) {
            alert("联系方式请不要超过50字");
        }
    });
    $("#submit-btn").click(function(){
        var suggestion=$(".suggestion-textarea  textarea").val();
        var contact=$(".suggestion-input input").val();
        if(suggestion==""){
            alert("反馈不能为空");
            return false;
        }
    ...
    /*使用帮助 打开问题详情页*/
    var boo;
    $(".help-list-content").click(function(){
        if($(this).find(".answer").css("display")=="block"){
            $(this).find(".help-goEdit").css("transform","rotateZ("+0+"deg)");
            $(this).find(".answer").slideUp(500);
        }else{
            $(".help-list").siblings().find(".help-goEdit").css("transform","rotateZ("+0+"deg)");
            $(this).find(".help-goEdit").css("transform","rotateZ("+90+"deg)");
            $(".help-list").siblings().find(".answer").slideUp();
